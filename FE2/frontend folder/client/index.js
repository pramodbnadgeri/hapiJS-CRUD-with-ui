async function handleFind() {
  ...
  if (!phone) {
    ...
    // fetch data
    let data = await fetch(`http://localhost:3000/users/${phone}`, {
      headers: {
...
async function handlePost() {
  ...
  if (!name || !phone || !address) {
    ...
    // fetch data  post
    let data = await fetch("http://localhost:3000/log", {
      method: "POST",
...
async function handlePut() {
  ...
  if (!name || !phone || !address || !searchname) {
    ...
    let data = await fetch(`http://localhost:3000/update/${searchname}`, {
      method: "PUT",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
...
async function handleDelete() {
  ...
  if (!searchname) {
    ...
    let data = await fetch(`http://localhost:3000/del/${searchname}`, {
      method: "DELETE",
      headers: {
        Accept: "application/json",
        "Content-Type": "application/json",
